...
| methods                                  | explain                                                                 |
|:-----------------------------------------|:------------------------------------------------------------------------|
| `static boolean equals(Objct a, Object b)` | 如果 a, b 都为 null, 则返回 true; 如果只有其中一个位 null, 则返回 false; 否则返回 a.equals(b); |
| `static it hash(Object... objects)` 7      | 返回一个 散列码, 由提供的所有对象的散列码组合得到                                              |
| `static int hashCode(Object a)` 7          | 如果 a 为 null, 返回 0; 否则返回 a.hashCode()                                    |
...
| methods                                         | explanation                                                                   |
|:------------------------------------------------|:------------------------------------------------------------------------------|
| `static Boolean equals(type[] a, type[] b)` 5.0 | 如果两个数组长度和元素都相同, 则返回 true; 在 Object[] 对象数组中, 如果都为 null 则返回 true; 否则返回 false;zz |
| `static int hashCode(type[] a)` 5.0             | 计算素组 a 的散列码                                                                   |
